...
case ${joint_mode} in
    DB )
        ...
        ${GATK4} --java-options "${java_opt2x} -XX:+UseSerialGC -Dsamjdk.use_async_io_write_samtools=false -DGATK_STACKTRACE_ON_USER_EXCEPTION=true" GenomicsDBImport --gatk-config-file /home/cocca/scripts/GATKConfig.EXAMPLE.properties --genomicsdb-workspace-path ${fol7}/${variantdb}/dbImport_${f2} --batch-size ${bs} -L "${f1}" --sample-name-map ${fol7}/${variantdb}/gVCF.list --reader-threads ${rt} -ip ${ip2} --tmp-dir ${tmp}
        echo "- END -"
    ;;
    GENO )
        echo
