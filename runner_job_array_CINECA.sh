...
# Environment variables: ${SLURM_ARRAY_TASK_ID}
#Example command:
...
while getopts ":dstql" opt; do
  case $opt in
    d)
      ...
      file=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3 | awk '{print $1}'`
      file2=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3 | awk '{print $2}'`
      echo ${file}
      echo ${file2}
      script=$2
      $script ${file} ${file2} "${@:4}"
    ...
    t)
      ...
      file1=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3 | awk '{print $1}'`
      file2=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3 | awk '{print $2}'`
      file3=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3 | awk '{print $3}'`
      echo ${file1}
      echo ${file2}
      echo ${file3}
    ...
    q)
      ...
      file2=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3 | awk '{print $2}'`
      file3=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3 | awk '{print $3}'`
      file4=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3 | awk '{print $4}'`
      echo ${file1}
      echo ${file2}
      echo ${file3}
      echo ${file4}
    ...
    s)
      ...
      file=`sed -n "${SLURM_ARRAY_TASK_ID}p" $3`
      echo ${file}
      script=$2
