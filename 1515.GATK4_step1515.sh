...
case ${joint_mode} in
    ...
    GENO )
        ...
        cd ${fol7}/
        echo "> GenotypeGVCFs"
        ${GATK4} --java-options ${java_opt2x} GenotypeGVCFs --dbsnp  -R ${GNMhg38} -O ${fol8}/${variantdb}/${int_vcf} -G StandardAnnotation --only-output-calls-starting-in-intervals --use-new-qual-calculator -V ${fol7}/${variantdb}/${f2}_g.vcf.gz -L "${f1}"
        echo "- END -"
    ;;
esac
