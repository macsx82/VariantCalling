...
if [[ ${whole_genome} -eq 1 ]]; then
	...
	${GATK4} --java-options "${java_opt2x}" BaseRecalibrator -R ${GNMhg38} -I ${fol1}/${fCRAM} --use-original-qualities --showHidden --tmp-dir ${tmp}/ -O ${fol3}/${c_bqsrrd} --known-sites ${DBSNP_latest} --known-sites ${INDELS} --known-sites ${OTGindels} -ip ${ip1} -L ${f1}
fi
