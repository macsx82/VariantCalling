...
#step 5
#IN fastq OUT val /// trim_galore
echo "bash \${hs}/05.preGATK4_step5.sh ${param_file}" | qsub -N pGs05_\${SM} -cwd -l h_vmem=\${seq_m} -hold_jid pGs04_\${SM} -o \${lg}/\$JOB_ID_pG05_\${SM}.log -e \${lg}/\\$JOB_ID_pG05_\${SM}.error -m a -M \${mail} -q \${seg_q}

#step 6
#IN val OUT fastqc /// fastqc
echo "bash \${hs}/06.preGATK4_step6.sh ${param_file}" | qsub -N pGs06_\${SM} -cwd -l h_vmem=\${seq_m} -hold_jid pGs05_\${SM} -o \${lg}/\$JOB_ID_pG06_\${SM}.log -e \${lg}/\\$JOB_ID_pG06_\${SM}.error -m ea -M \${mail} -q \${seg_q}
