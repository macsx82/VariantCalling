...
cd ${fol9}/${variantdb}/
echo "> Sample list"
grep "CHROM" ${raw} | sed 's/\t/\n/g' | tail -n +10 > "${variantdb}_g2626_sample.list"
echo "- END -"
...
while read -r SM ; do ${GATK4} --java-options ${java_opt2x} SelectVariants -R ${GNMhg38} -V xSamplePassedVariantsVCFs/"${variantdb}_${SM}_SampleOnlyPASS_Variants_PostVQSR.vcf" -O xSamplePassedVariantsIntervalFilteredVCFs_EXONSplus12/"${variantdb}_${SM}_SampleOnlyPASSandIntervalFiltered_EXONSplus12_Variants_PostVQSR.vcf" -L ${sorgILhg38exons12PlusINTERVALS} -sn ${SM}; done < "${variantdb}_g2626_sample.list"
echo "- END -"
...
echo -n "samples n. = "; wc -l "${variantdb}_g2626_sample.list" | awk '{print $1}'
echo -n "intervals n. = "; wc -l ${sorgILhg38exons12PlusINTERVALS} | awk '{print $1}'
echo
while read -r SM ; do echo -n "quite number of variants (including header) in ${SM} PRE-filtered VCF = "; wc -l xSamplePassedVariantsVCFs/"${variantdb}_${SM}_SampleOnlyPASS_Variants_PostVQSR.vcf" | awk '{print $1}'; echo -n "quite number of variants (including header) in ${SM} POST-filtered VCF = "; wc -l xSamplePassedVariantsIntervalFilteredVCFs_EXONSplus12/"${variantdb}_${SM}_SampleOnlyPASSandIntervalFiltered_EXONSplus12_Variants_PostVQSR.vcf" | awk '{print $1}'; echo; done < "${variantdb}_g2626_sample.list"
echo "- END -"
